...
app.get('/', function(req, res, next){
	if(req.cookies.id){
		var id;
		var query = client.query('select id from login where id='+req.cookies.id);
		query.on('row', function(row){
			id = row;
		});
		query.on('end', function(){
			if(id) login(req.cookies.id, res, next)
...
app.post('/html/signup', function(req, res, next){
	...
	// client.query('create table if not exists "'+uniqueTable+'"(post varchar(1000), moment date, name varchar(100))');
...
app.get('/html/currentUserInfo', function(req, res, next){
	var cookie = req.cookies, post = [];
	if(cookie.id){
		var id = req.cookies.id, user;
		var query1 = client.query("select name, content, moment from posts where userId ='"+id+"'", function(err){
		...
		query1.on('end', function(){
			var query2 = client.query("select followers from login where id ='"+id+"'"), followers;
			query2.on('row', function(row){
				followers = row.followers || JSON.stringify([]);
			});	
			query2.on('end', function(){
				followers = JSON.parse(followers);
				followers.forEach(function(id){
					var query3 = client.query("select name, content, moment from posts where userId ='"+id+"'");
