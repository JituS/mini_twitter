...
app.post('/html/newPost', function(req, res, next){
	...
	if(cookie.id){
		client.query('create table if not exists posts(postId serial primary key, userId varchar(20), name varchar(100), content varchar(1000), moment date)');
...
app.post('/html/like', function(req, res, next){
	client.query('create table if not exists likes(postId serial REFERENCES posts(postId), userId varchar(20))');
});
