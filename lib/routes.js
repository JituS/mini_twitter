...
app.post('/html/signup', function(req, res, next){
	...
	client.query('insert into login values('+unique+','+name+','+email+','+DOB+','+password+')', function(err, err){
...
app.get('/html/currentUserInfo', function(req, res, next){
	...
	if(cookie.id){
		...
		var query1 = client.query("select postId, name, content, moment from posts where userId ='"+id+"'", function(err, result){
			...
			query2.on('end', function(){
				if(followers){
					...
					for(var i = 0; i < followers.length; i++){
						var query3 = client.query("select postId, name, content, moment from posts where userId ='"+followers[i]+"'");
...
app.post('/html/like', function(req, res, next){
	...
	console.log("update posts set likes = likes+1, likers = likers|| "+userId+"||"+"\' "+"\'"+" where postid="+postId);
